...
namespace light
{
    ...
    public:
		...
		template<typename sink_type>
		void InitPersistSink(bool is_auto_flush, const std::string& path);
    };
    ...
    template<typename sink_type>
    void Logger::impl::InitPersistSink(bool is_auto_flush, const std::string& path)
    {
        /*@ asynchronous sink to file.*/
        // construct backend.
        boost::shared_ptr<sinks::text_file_backend> backend =
            boost::make_shared<sinks::text_file_backend>(
                keywords::open_mode = std::ios::app,
                keywords::file_name = path + "/%Y%m%d_%N.log",
                keywords::rotation_size = 500 * 1024 * 1024,        // 500M
                keywords::time_based_rotation =
                sinks::file::rotation_at_time_point(0, 0, 0));
        backend->auto_flush(is_auto_flush);
    ...
    void Logger::InitPersistLog(bool is_sync, bool is_auto_flush, const std::string& path)
    {
        if (is_sync)
        {
            m_impl->InitPersistSink<impl::sync_file_sink>(is_auto_flush, path);
        }
        else
        {
            m_impl->InitPersistSink<impl::async_file_sink>(is_auto_flush, path);
        }
    }
